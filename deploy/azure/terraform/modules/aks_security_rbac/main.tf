...
resource "azurerm_role_assignment" "github_actions_reader_subscription" {
  count                = var.enable_rbac_role_assignments && var.enable_github_actions_role_assignments && !var.enable_github_actions_subscription_contributor ? 1 : 0
...
resource "azurerm_role_assignment" "github_actions_user_access_administrator_subscription" {
  count                = var.enable_rbac_role_assignments && var.enable_github_actions_role_assignments && var.enable_github_actions_user_access_administrator ? 1 : 0
  principal_id         = data.azuread_service_principal.github_actions[0].object_id
