...
function TerminalModal({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-12 animate-in fade-in duration-300">
      <div
        className="absolute inset-0 bg-kube-dark/80 backdrop-blur-sm"
        onClick={onClose}
      ></div>
        ...
        <div className="flex-1 overflow-auto p-6 font-mono text-sm leading-relaxed selection:bg-kube-blue/30">
          {loading ? (
            <div className="flex flex-col items-center justify-center h-full space-y-4">
              ...
              <p className="text-white/20 animate-pulse">
                Fetching data from K8s API...
              </p>
            </div>
          ) : (
  ...
  return (
    ...
    >
      {copied ? (
        ...
        <Copy size={14} className="text-white/40 group-hover/copy:text-white" />
      )}
      {label && (
        <span className="text-[10px] uppercase font-bold text-white/40 group-hover/copy:text-white">
          {label}
        </span>
      )}
  ...
  return (
    <div className="space-y-4">
      {events.map((e, i) => (
        <div
          key={i}
          className="group p-4 bg-white/5 border border-white/10 rounded-xl hover:bg-white/10 transition-all relative overflow-hidden text-left"
        >
          <div
            className={`absolute left-0 top-0 h-full w-1 ${e.type === "Warning" ? "bg-amber-500 shadow-[0_0_10px_rgba(245,158,11,0.5)]" : "bg-emerald-500/50"}`}
          ></div>
          <div className="flex justify-between items-start">
            <div className="flex items-start space-x-3">
              <div
                className={`mt-1 w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${e.type === "Warning" ? "bg-amber-500/10 border border-amber-500/20" : "bg-emerald-500/10 border border-emerald-500/20"}`}
              >
                {e.type === "Warning" ? (
              ...
              <div>
                <div className="flex items-center space-x-2">
                  <span
                    className={`text-[10px] font-black uppercase tracking-widest px-1.5 py-0.5 rounded ${e.type === "Warning" ? "bg-amber-500/20 text-amber-500" : "bg-emerald-500/20 text-emerald-500"}`}
                  >
                    {e.reason}
                  </span>
                ...
                <div className="flex items-center space-x-3 mt-2 text-[10px] text-white/20 font-bold uppercase tracking-tighter">
                  ...
                  <span>
                    {e.last_timestamp
                      ? new Date(e.last_timestamp).toLocaleString([], {
                          hour: "2-digit",
                          minute: "2-digit",
                          second: "2-digit",
                        })
...
const resourceDescriptions: Record<
  string,
...
> = {
  ...
  "storage/pvcs": {
    ...
    description:
      "A request for storage by a user, similar to a Pod requesting node resources.",
    icon: <Database size={18} className="text-amber-400" />,
  },
  "config/configmaps": {
    title: "ConfigMaps",
    description:
      "This is where your application environment variables (app_env) and non-confidential configuration files are stored. Pods inject these at runtime as environment variables or mounted files.",
    icon: <FileText size={18} className="text-blue-400" />,
  },
  "config/secrets": {
    title: "Secrets",
    description:
      "This is the source for sensitive application variables like database credentials, API keys, and OAuth tokens. They are encoded by default and can be injected into your pods securely.",
    icon: <Lock size={18} className="text-red-400" />,
  },
  "mesh/virtualservices": {
    title: "Istio VirtualServices",
    description:
      "Define a set of traffic routing rules to apply when a host is addressed.",
    icon: <ShieldCheck size={18} className="text-kube-blue" />,
  },
  "mesh/gateways": {
    title: "Istio Gateways",
    description:
      "Describe a load balancer operating at the edge of the mesh receiving incoming or outgoing HTTP/TCP connections.",
    icon: <ShieldCheck size={18} className="text-kube-blue" />,
  },
  ...
  "workloads/statefulsets": {
    title: "StatefulSets",
    description:
      "Manages the deployment and scaling of a set of Pods, and provides guarantees about the ordering and uniqueness of these Pods. These are essential for stateful applications like Databases.",
    icon: <Server size={18} className="text-emerald-400" />,
  },
  "workloads/daemonsets": {
    title: "DaemonSets",
    description:
      "Ensures that all (or some) Nodes run a copy of a Pod. These are typically used for background tasks like logging agents or monitoring systems that must exist on every server.",
    icon: <Activity size={18} className="text-amber-400" />,
  },
  "workloads/hpas": {
    title: "Horizontal Pod Autoscalers",
    description:
      "Automatically scales the number of Pods in a replication controller, deployment, replica set or stateful set based on observed CPU utilization or other metrics.",
    icon: <Zap size={18} className="text-kube-blue" />,
  },
  "networking/ingress": {
    title: "Ingress",
    description:
      "An API object that manages external access to the services in a cluster, typically HTTP. Ingress provides load balancing, SSL termination and name-based virtual hosting.",
    icon: <Globe size={18} className="text-kube-blue" />,
  },
  "networking/networkpolicies": {
    title: "Network Policies",
    description:
      "Specifications of how groups of pods are allowed to communicate with each other and other network endpoints. This is the L3/L4 firewall for your Kubernetes namespace.",
    icon: <ShieldCheck size={18} className="text-amber-400" />,
  },
  "rbac/serviceaccounts": {
    title: "ServiceAccounts",
    description:
      "Provides an identity for processes that run in a Pod. Applications use these to securely interact with the Kubernetes API server.",
    icon: <Lock size={18} className="text-emerald-400" />,
  },
  "rbac/roles": {
    title: "Roles",
    description:
      "Contains rules that represent a set of permissions within a namespace. Roles are scope-restricted to the current namespace.",
    icon: <ShieldCheck size={18} className="text-emerald-400" />,
  },
  "rbac/rolebindings": {
    title: "RoleBindings",
    description:
      "Grants the permissions defined in a role to a user or set of users. Linking a Role to a ServiceAccount is done here.",
    icon: <ShieldCheck size={18} className="text-blue-400" />,
  },
  "rbac/clusterroles": {
    title: "ClusterRoles",
    description:
      "A cluster-level resource that can be used to grant permissions across the entire cluster. Often used for shared resources like Nodes or across all namespaces.",
    icon: <ShieldCheck size={18} className="text-red-400" />,
  },
  "rbac/clusterrolebindings": {
    title: "ClusterRoleBindings",
    description:
      "Grants permissions cluster-wide to a user or set of users. This is used for powerful administrative or cross-namespace permissions.",
    icon: <ShieldCheck size={18} className="text-red-500" />,
  },
  "infra/certificates": {
    title: "Certificates (SSL/TLS Hub)",
    description:
      "A centralized view of Certificate Signing Requests (CSRs). This is where you approve and manage SSL/TLS certificates for secure pod communication.",
    icon: <ShieldCheck size={18} className="text-blue-400" />,
  },
  "infra/helm": {
    title: "Helm Chart Designer",
    description:
      "Visual architect for defining your infrastructure. Scaffold production-ready charts with Deployments, Services, and Ingress in a standardized way.",
    icon: <Cloud size={18} className="text-purple-400" />,
  },
  "observability/traffic": {
    title: "Traffic Routing (Live View)",
    description:
      "Real-time visualization of the Istio service mesh traffic. Monitor how requests flow from the Ingress Controller to your Pods.",
    icon: <Globe size={18} className="text-kube-blue" />,
  },
};
    ...
    <div className="col-span-full py-20 px-6 bg-white/5 border border-dashed border-white/10 rounded-3xl flex flex-col items-center justify-center text-center space-y-4">
      ...
      <div>
        ...
        <p className="text-sm text-white/20 max-w-sm mx-auto">
          Either there are no resources in this namespace, or they haven't been
          created yet. Check your kubectl context or switch namespaces.
        </p>
      </div>
    </div>
  );
...
export default function Home() {
  ...
  const currentContextObj =
    contexts.find((c) => c.name === activeContext) || null;
  const activeCluster = currentContextObj?.cluster || "Searching...";
  const activeServer = currentContextObj?.server || "Loading...";

  // Helm Designer State (Canonical for base-api-chart)
  const [helmConfig, setHelmConfig] = useState<any>({
    replicaCount: 1,
    image: {
      repository: "nginx",
    ...
    env: {
      APP_ENV_VAR: "Hello from KUUI Designer!",
    },
    secrets: {
      DB_PASSWORD: "change-me-base64",
    },
    serviceAccount: {
      create: true,
      name: "",
  ...
  const fetchData = async (ns: string = selectedNamespace) => {
    ...
    try {
      ...
      const results = await Promise.all(
        endpoints.map((e) =>
          fetch(`${apiUrl}/resources/${e.url}?namespace=${ns}`).then((r) =>
            r.json(),
          ),
        ),
      );
      ...
      const eventsRes = await fetch(
        `${apiUrl}/resources/events?namespace=${ns}`,
      );
  ...
  const handleSwitchContext = async (contextName: string) => {
    ...
    try {
      const res = await fetch(`${apiUrl}/contexts/switch?name=${contextName}`, {
        method: "POST",
      });
      if (res.ok) {
  ...
  const handleFetchLogs = async (podName: string) => {
    ...
    try {
      const res = await fetch(
        `${apiUrl}/resources/pods/${podName}/logs?namespace=${selectedNamespace}`,
      );
      const data = await res.json();
  ...
  const handleDeleteContext = async (name: string) => {
    if (
      !window.confirm(
        `Are you sure you want to delete context '${name}'? This will permanently modify your kubeconfig.`,
      )
    ) {
      return;
    }
    ...
    try {
      const res = await fetch(`${apiUrl}/contexts/${name}`, {
        method: "DELETE",
      });
      const data = await res.json();
      ...
      alert("Failed to connect to backend for context deletion.");
    }
  };
  ...
  const handleDescribePod = async (podName: string) => {
    ...
    try {
      const res = await fetch(
        `${apiUrl}/resources/pods/${podName}/describe?namespace=${selectedNamespace}`,
      );
      const data = await res.json();
      ...
      <header className="h-16 border-b border-white/10 bg-kube-dark/80 backdrop-blur-md flex items-center justify-between px-6 sticky top-0 z-50">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-xl font-bold tracking-tight">
              KUUI{" "}
              <span className="text-kube-blue font-black italic">MASTER</span>
            </h1>
        ...
        <div className="flex items-center space-x-4">
          ...
          <div className="flex items-center space-x-2 px-3 py-1.5 bg-white/5 border border-white/10 rounded-lg group relative hover:border-kube-blue/30 transition-all">
            <span className="text-[10px] uppercase font-bold text-white/40 absolute -top-2 left-2 bg-kube-dark px-1">
              Cluster / Context
            </span>
            ...
            >
              {contexts.map((ctx) => (
                <option
                  key={ctx.name}
                  value={ctx.name}
                  className="bg-kube-dark text-white"
                >
          ...
          <div className="flex items-center space-x-2 px-3 py-1.5 bg-white/5 border border-white/10 rounded-lg group relative hover:border-kube-blue/30 transition-all">
            ...
            >
              {namespaces.map((ns) => (
                <option key={ns} value={ns} className="bg-kube-dark text-white">
                  {ns}
                </option>
              ))}
            </select>
          ...
          <div className="relative group">
            <Search
              className="absolute left-3 top-1/2 -translate-y-1/2 text-white/40 group-focus-within:text-kube-blue transition-colors"
              size={18}
            />
            <input
              type="text"
      ...
      <div className="flex flex-1">
        ...
        <aside className="w-64 border-r border-white/10 p-4 space-y-6 hidden md:block overflow-y-auto">
          <nav className="space-y-4">
            <SidebarCategory
              ...
              onToggle={() =>
                setExpandedCategories((prev) =>
                  prev.includes("workloads")
                    ? prev.filter((c) => c !== "workloads")
                    : [...prev, "workloads"],
                )
              }
            >
              <SidebarItem
                label="Pods"
                active={activeTab === "workloads/pods"}
                onClick={() => setActiveTab("workloads/pods")}
              />
              <SidebarItem
                label="Deployments"
                active={activeTab === "workloads/deployments"}
                onClick={() => setActiveTab("workloads/deployments")}
              />
              <SidebarItem
                label="StatefulSets"
                active={activeTab === "workloads/statefulsets"}
                onClick={() => setActiveTab("workloads/statefulsets")}
              />
              <SidebarItem
                label="DaemonSets"
                active={activeTab === "workloads/daemonsets"}
                onClick={() => setActiveTab("workloads/daemonsets")}
              />
              <SidebarItem
                label="HPA (Scaling)"
                active={activeTab === "workloads/hpas"}
                onClick={() => setActiveTab("workloads/hpas")}
              />
            </SidebarCategory>
            ...
            <SidebarCategory
              ...
              onToggle={() =>
                setExpandedCategories((prev) =>
                  prev.includes("networking")
                    ? prev.filter((c) => c !== "networking")
                    : [...prev, "networking"],
                )
              }
            >
              <SidebarItem
                label="Services"
                active={activeTab === "networking/services"}
                onClick={() => setActiveTab("networking/services")}
              />
              <SidebarItem
                label="Ingress"
                active={activeTab === "networking/ingress"}
                onClick={() => setActiveTab("networking/ingress")}
              />
              <SidebarItem
                label="Network Policies"
                active={activeTab === "networking/networkpolicies"}
                onClick={() => setActiveTab("networking/networkpolicies")}
              />
            </SidebarCategory>
            ...
            >
              <SidebarItem
                ...
                active={activeTab === "rbac/serviceaccounts"}
                onClick={() => setActiveTab("rbac/serviceaccounts")}
              />
              <SidebarItem
                label="Roles"
                active={activeTab === "rbac/roles"}
                onClick={() => setActiveTab("rbac/roles")}
              />
              <SidebarItem
                label="RoleBindings"
                active={activeTab === "rbac/rolebindings"}
                onClick={() => setActiveTab("rbac/rolebindings")}
              />
              <SidebarItem
                label="ClusterRoles"
                active={activeTab === "rbac/clusterroles"}
                onClick={() => setActiveTab("rbac/clusterroles")}
              />
              <SidebarItem
                label="ClusterRoleBindings"
                active={activeTab === "rbac/clusterrolebindings"}
                onClick={() => setActiveTab("rbac/clusterrolebindings")}
              />
            </SidebarCategory>
            ...
            >
              <SidebarItem
                ...
                active={activeTab === "storage/pvcs"}
                onClick={() => setActiveTab("storage/pvcs")}
              />
            </SidebarCategory>
            ...
            >
              <SidebarItem
                label="ConfigMaps"
                active={activeTab === "config/configmaps"}
                onClick={() => setActiveTab("config/configmaps")}
              />
              <SidebarItem
                label="Secrets"
                active={activeTab === "config/secrets"}
                onClick={() => setActiveTab("config/secrets")}
              />
            </SidebarCategory>
            ...
            <SidebarCategory
              ...
              onToggle={() =>
                setExpandedCategories((prev) =>
                  prev.includes("infra")
                    ? prev.filter((c) => c !== "infra")
                    : [...prev, "infra"],
                )
              }
            >
              <SidebarItem
                label="Certificates (SSL Hub)"
                active={activeTab === "infra/certificates"}
                onClick={() => setActiveTab("infra/certificates")}
              />
              <SidebarItem
                label="Helm Designer"
                active={activeTab === "infra/helm"}
                onClick={() => setActiveTab("infra/helm")}
              />
            </SidebarCategory>
            ...
            >
              <SidebarItem
                ...
                active={activeTab === "mesh/virtualservices"}
                onClick={() => setActiveTab("mesh/virtualservices")}
              />
              <SidebarItem
                label="Gateways"
                active={activeTab === "mesh/gateways"}
                onClick={() => setActiveTab("mesh/gateways")}
              />
            </SidebarCategory>
            ...
            <SidebarCategory
              ...
              onToggle={() =>
                setExpandedCategories((prev) =>
                  prev.includes("observability")
                    ? prev.filter((c) => c !== "observability")
                    : [...prev, "observability"],
                )
              }
            >
              <SidebarItem
                label="Live Traffic"
                active={activeTab === "observability/traffic"}
                onClick={() => setActiveTab("observability/traffic")}
              />
            </SidebarCategory>
          </nav>
        </aside>
        ...
        <main className="flex-1 p-8 space-y-8 overflow-y-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <StatCard
              icon={<Cpu className="text-kube-blue" />}
              label="Nodes"
              value={clusterOverview?.total_nodes || "0"}
              subValue={`${clusterOverview?.healthy_nodes || 0} Healthy`}
            />
            <StatCard
              icon={<Server className="text-purple-400" />}
              label="API Version"
              value={clusterOverview?.api_version || "N/A"}
              subValue={clusterOverview?.platform || "K8s Cluster"}
            />
            <StatCard
              icon={<Box className="text-emerald-400" />}
              label="Avg Pod Health"
              value={clusterOverview?.pod_health_pct || "0%"}
              subValue={`Across ${clusterOverview?.total_pods || 0} total pods`}
            />
            <StatCard
              icon={<Globe className="text-amber-400" />}
              label="Active Cluster"
              value={activeCluster}
              subValue={activeServer}
            />
          </div>
          ...
          {switchingNamespace ? (
            <div className="flex-1 flex flex-col items-center justify-center min-h-[60vh] animate-in fade-in duration-700">
              ...
              <div className="mt-8 space-y-2 text-center">
                <h2 className="text-2xl font-bold tracking-tight text-white/80 animate-pulse">
                  Switching Namespace...
                </h2>
                <p className="text-white/30 text-sm font-medium uppercase tracking-[0.2em]">
                  Synchronizing {selectedNamespace} resources
                </p>
              </div>
            </div>
          ) : (
            <>
              {activeTab === "workloads/pods" && (
                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 2xl:grid-cols-3 gap-6">
                    {(loading ? Array(6).fill(0) : pods).length > 0 ? (
                      (loading ? Array(6).fill(0) : pods).map(
                        (pod: any, i: number) =>
                          loading ? (
              ...
              {activeTab === "rbac/clusterrolebindings" && (
                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  <ResourceHeader
                    {...resourceDescriptions["rbac/clusterrolebindings"]}
                  />
                  <div className="grid grid-cols-1 lg:grid-cols-2 2xl:grid-cols-3 gap-6">
                    {loading ? (
                      <LoadingSkeleton />
                ...
                <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        ...
                        <div className="col-span-full p-12 bg-white/5 border border-dashed border-white/10 rounded-3xl flex flex-col items-center justify-center text-center space-y-4">
                          ...
                          <div>
                            <p className="text-white/60 font-bold">
                              No active CSRs found
                            </p>
                            <p className="text-xs text-white/20">
                              Generate a workload CSR to see it appear here for
                              approval.
                            </p>
                          </div>
                        </div>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 2xl:grid-cols-3 gap-6">
                      ...
                      {secrets.filter((s) => s.type === "kubernetes.io/tls")
                        .length === 0 && (
              ...
              {activeTab === "infra/helm" && (
                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  ...
                  <p className="text-sm text-white/40 max-w-2xl">
                    Design your infrastructure visually. Toggle modules like
                    Ingress, Storage, and Secrets to generate a standardized
              ...
              {activeTab === "mesh/virtualservices" && (
                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 2xl:grid-cols-3 gap-6">
                    {loading ? (
                      <LoadingSkeleton />
                    ) : istioVirtualServices.length > 0 ? (
                      istioVirtualServices.map((vs: any) => (
                        <IstioCard
                          key={vs.name}
                          {...vs}
                          type="VIRTUALSERVICE"
              ...
              {activeTab === "mesh/gateways" && (
                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 2xl:grid-cols-3 gap-6">
                    {loading ? (
                      <LoadingSkeleton />
                    ) : istioGateways.length > 0 ? (
                      istioGateways.map((gw: any) => (
                        <IstioCard
                          key={gw.name}
                          {...gw}
                          type="GATEWAY"
                    ...
                    <div className="grid grid-cols-1 2xl:grid-cols-3 gap-8">
                      ...
                      <div className="space-y-6">
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <span className="text-[10px] text-white/10 font-bold uppercase tracking-tighter">
                            Last 50
                          </span>
                        </div>
...
function KconfigModal({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      <div
        className="absolute inset-0 bg-kube-dark/80 backdrop-blur-md"
        onClick={onClose}
      ></div>
          ...
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h2 className="text-2xl font-bold text-white tracking-tight">
                KubeConfig <span className="text-purple-400">Audit</span>
              </h2>
              <p className="text-sm text-white/40">
                Inspect all available Kubernetes cluster connections and
                environments.
              </p>
            </div>
          </div>
            ...
            {contexts.map((ctx) => (
              ...
              >
                ...
                <div className="flex items-start justify-between">
                  <div className="space-y-1">
                    <div className="flex items-center space-x-3">
                      <h3 className="text-lg font-bold text-white group-hover:text-kube-blue transition-colors">
                        {ctx.cluster}
                      </h3>
                    ...
                    <p className="text-xs text-white/40 font-mono tracking-tighter">
                      {ctx.server}
                    </p>
                  </div>
                  <div className="flex flex-col items-end space-y-4">
                    ...
                    {ctx.name !== activeContext && (
                      <button
                        ...
                        className="p-2 bg-red-500/10 border border-red-500/20 rounded-xl text-red-400 hover:bg-red-500/20 hover:text-red-300 transition-all shadow-lg"
                ...
                <div className="mt-4 flex items-center space-x-6">
                  <div className="flex items-center space-x-2">
                    <div
                      className={`w-2 h-2 rounded-full ${ctx.name === activeContext ? "bg-kube-blue" : "bg-white/10"}`}
                    ></div>
                    <span className="text-[10px] font-bold uppercase tracking-widest text-white/40">
                      Status:{" "}
                      {ctx.name === activeContext ? "Connected" : "Available"}
                    </span>
                  </div>
                  <div className="flex items-center space-x-2">
...
function ContextItem({
  ...
  return (
    ...
    >
      <div
        className={`w-2 h-2 rounded-full ${active ? "bg-kube-blue animate-pulse" : "bg-white/20"}`}
      ></div>
      <span>{label}</span>
    </div>
  );
...
function StatCard({
  ...
  const tooltipContent =
    {
      Nodes:
        "Physical or virtual machines in your cluster that run containerized applications.",
      "API Version":
        "The specific version of the Kubernetes API server running on the control plane.",
      "Avg Pod Health":
        "Percentage of pods currently in the 'Running' or 'Succeeded' phase across all namespaces.",
      "Active Context":
        "The current kube-config context being used by the KUUI backend proxy.",
  ...
  return (
    <div className="p-5 bg-white/5 border border-white/10 rounded-2xl space-y-2 hover:border-white/20 transition-all group relative">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="text-xs text-white/40 bg-white/5 px-2 py-1 rounded-full group-hover:text-white/60 transition-colors uppercase tracking-widest font-bold">
          Live
        </div>
      </div>
      <div>
        <p className="text-sm text-white/40 font-medium tracking-wide uppercase leading-relaxed">
          {label}
        </p>
      ...
      <p className="text-xs text-white/30 italic group-hover:text-white/50 transition-colors">
        {subValue}
      </p>
    </div>
  );
...
function PodCard({
  ...
  return (
    ...
    >
      <div
        className={`absolute top-0 left-0 w-1 h-full ${isHealthy ? "bg-kube-blue shadow-lg shadow-kube-blue/20" : "bg-red-500 animate-pulse-slow shadow-lg shadow-red-500/40"}`}
      ></div>
      ...
      {(!isHealthy || message) && (
        <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity z-10 pointer-events-none">
          <div className="bg-black/90 backdrop-blur-md px-3 py-2 rounded-lg border border-red-500/30 text-[10px] max-w-[200px] shadow-2xl">
            ...
            <p className="text-white/60 leading-tight italic">
              "{message || `Pod is in ${status} state.`}"
            </p>
          </div>
        </div>
      )}
      ...
      <div className="flex justify-between items-start mb-4">
        <div className="flex items-center space-x-3">
          <div
            className={`w-12 h-12 rounded-xl flex items-center justify-center transition-all shadow-inner border ${!isHealthy ? "bg-red-500/10 border-red-500/20 group-hover:scale-110" : "bg-kube-blue/10 border-kube-blue/20 group-hover:scale-110"}`}
          >
            <Box
              className={!isHealthy ? "text-red-400" : "text-kube-blue"}
              size={24}
            />
          </div>
          <div>
            <div className="flex items-center space-x-2">
              <h3
                className={`font-bold text-base leading-tight transition-colors break-all ${!isHealthy ? "text-red-400 group-hover:text-red-300" : "group-hover:text-kube-blue"}`}
              >
                {name}
              </h3>
        ...
        <div
          className={`px-2 py-1 rounded-md text-[10px] font-black uppercase tracking-widest border transition-colors ${isRunning ? "text-emerald-400/80 border-emerald-400/20 bg-emerald-400/5" : "text-red-400/80 border-red-400/20 bg-red-400/5"}`}
        >
          {reason || status}
        </div>
      </div>
      ...
      {restarts > 0 && (
        <div className="mb-4">
          <div className="flex items-center justify-between text-[10px] mb-1">
            <span className="text-white/20 uppercase font-bold tracking-widest">
              Restart Count
            </span>
          ...
          <div className="h-1 w-full bg-white/5 rounded-full overflow-hidden">
            <div
              ...
              style={{ width: `${Math.min(restarts * 10, 100)}%` }}
            ></div>
          </div>
        </div>
      )}
      ...
      <div className="flex space-x-2 pt-2 opacity-100 group-hover:translate-y-0 translate-y-2 transition-all">
        <button
          ...
          className="flex-1 px-3 py-1.5 bg-white/5 rounded-lg text-[10px] font-black uppercase tracking-tighter hover:bg-white/10 border border-white/5 transition-all hover:text-kube-blue"
        >
          Logs
        </button>
        <button
...
function DeploymentCard({
  ...
  return (
    <div className="p-6 bg-white/5 border border-white/10 rounded-2xl space-y-4 hover:bg-white/10 transition-all cursor-pointer relative group overflow-hidden">
      <div
        className={`absolute top-0 left-0 w-1 h-full ${isHealthy ? "bg-emerald-500 shadow-lg shadow-emerald-500/20" : "bg-red-500 animate-pulse"}`}
      ></div>
      <div className="flex justify-between items-start">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-white/40 font-mono font-bold tracking-widest uppercase truncate max-w-[150px]">
              {image}
            </p>
          </div>
        </div>
      ...
      <div className="flex items-center justify-between text-xs pt-1">
        <span className="text-white/40">
          Ready:{" "}
          <span
            className={`font-bold ${isHealthy ? "text-emerald-400" : "text-red-400"}`}
          >
      ...
      <div className="flex justify-between items-start">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-white/40 font-mono font-bold tracking-widest uppercase">
              {type}
            </p>
          </div>
        </div>
      ...
      <div className="space-y-2">
        ...
        <div className="flex flex-wrap gap-1.5 pt-1">
          {ports.map((p, i) => (
            <div
              key={i}
              className="px-2 py-0.5 bg-white/5 rounded border border-white/10 text-[9px] font-mono text-emerald-400/80"
            >
              {p.port}:{p.target_port}{" "}
              <span className="text-white/20 text-[8px] uppercase">
                {p.protocol}
              </span>
            </div>
          ))}
...
function StatefulSetCard({
  ...
  return (
    <div className="p-6 bg-white/5 border border-white/10 rounded-2xl space-y-4 hover:bg-white/10 transition-all cursor-pointer relative group overflow-hidden">
      <div
        className={`absolute top-0 left-0 w-1 h-full ${isHealthy ? "bg-emerald-500 shadow-lg shadow-emerald-500/20" : "bg-amber-500 animate-pulse"}`}
      ></div>
      <div className="flex justify-between items-start">
        <div className="flex items-center space-x-3">
      ...
      <div className="flex items-center justify-between text-xs pt-2">
        <span className="text-white/40">
          Ready:{" "}
          <span
            className={`font-bold ${isHealthy ? "text-emerald-400" : "text-amber-400"}`}
          >
...
function HPACard({
  ...
  return (
    <div className="p-6 bg-white/5 border border-white/10 rounded-2xl space-y-4 hover:bg-white/10 transition-all cursor-pointer relative group overflow-hidden">
      <div
        className={`absolute top-0 left-0 w-1 h-full ${isTargetMet ? "bg-kube-blue shadow-lg" : "bg-amber-500 animate-pulse"}`}
      ></div>
      <div className="flex justify-between items-start">
        <div className="flex items-center space-x-3">
      ...
      <div className="space-y-3 pt-1">
        ...
        <div className="space-y-1">
          <div className="flex items-center justify-between text-[10px]">
            <span className="text-white/20 uppercase font-bold">
              CPU Utilization
            </span>
            <span
              className={`font-bold ${cpuPct > targetPct ? "text-amber-500" : "text-kube-blue"}`}
            >
              {cpuPct}%
            </span>
          </div>
          <div className="h-1.5 w-full bg-white/5 rounded-full overflow-hidden border border-white/5">
            <div
              ...
              style={{
                width: `${Math.min((cpuPct / (targetPct || 100)) * 100, 100)}%`,
              }}
            ></div>
          </div>
        </div>
...
function RBACCard({
  ...
  return (
    <div
      className={`p-6 bg-white/5 border rounded-2xl space-y-4 hover:bg-white/10 transition-all cursor-pointer relative group overflow-hidden ${name.startsWith("system:") ? "opacity-50 grayscale hover:grayscale-0 transition-all" : "border-white/10"}`}
    >
      <div
        className={`absolute top-0 left-0 w-1 h-full bg-current ${colorClass.replace("border-", "bg-")}`}
      ></div>
      <div className="flex justify-between items-start">
        <div className="flex items-center space-x-3">
      ...
      <div className="pt-2 border-t border-white/5 space-y-2">
        {type === "SERVICEACCOUNT" && (
          <p className="text-[10px] text-white/40 uppercase font-bold">
            Secrets: <span className="text-white">{secrets_count || 0}</span>
          </p>
        )}
        {(type === "ROLE" || type === "CLUSTERROLE") && (
          <p className="text-[10px] text-white/40 uppercase font-bold">
            Rules: <span className="text-white">{rules_count || 0}</span>
          </p>
        )}
        {(type === "ROLEBINDING" || type === "CLUSTERROLEBINDING") && (
          <>
            <p className="text-[10px] text-white/40 uppercase font-bold">
              Ref: <span className="text-blue-400">{role_ref}</span>
            </p>
            <div className="flex flex-wrap gap-1 pt-1">
              {subjects?.map((s, i) => (
                <div
                  key={i}
                  className="px-2 py-0.5 bg-white/5 rounded border border-white/10 text-[9px] font-mono text-white/60"
                >
                  <span className="text-white/20 mr-1">{s.kind}:</span>
                  {s.name}
                </div>
              ))}
            </div>
      ...
      <div className="space-y-3">
        <div className="flex flex-wrap gap-2">
          {hosts.map((h, i) => (
            <div
              key={i}
              className="px-2 py-1 bg-white/5 rounded text-[10px] font-mono text-kube-blue border border-white/5"
            >
              {h}
            </div>
          ))}
        ...
        {Object.keys(annotations).length > 0 && (
          <div className="pt-2 border-t border-white/5">
            <p className="text-[8px] uppercase font-bold text-white/20 mb-1">
              Top Annotations
            </p>
            <div className="flex flex-wrap gap-1">
              {Object.entries(annotations)
                ...
                .map(([k, v], i) => (
                  <div
                    key={i}
                    className="text-[9px] text-white/40 truncate w-full flex items-center space-x-1"
                  >
  ...
  return (
    <div className="p-6 bg-white/5 border border-white/10 rounded-2xl space-y-4 hover:bg-white/10 transition-all cursor-pointer relative group overflow-hidden">
      ...
      <div className="pt-2 border-t border-white/5">
        ...
        <div className="flex flex-wrap gap-1">
          {Object.entries(pod_selector).length > 0 ? (
            Object.entries(pod_selector).map(([k, v], i) => (
              <div
                key={i}
                className="px-2 py-0.5 bg-white/5 rounded border border-white/10 text-[9px] font-mono text-amber-400/80"
              >
                {k}={String(v)}
              </div>
            ))
          ) : (
            <span className="text-[10px] italic text-white/20">
              Matches all pods (allow)
            </span>
          )}
        </div>
      </div>
...
function PVCCard({
  ...
  return (
    <div className="p-6 bg-white/5 border border-white/10 rounded-2xl space-y-4 hover:bg-white/10 transition-all cursor-pointer relative group overflow-hidden">
      <div
        className={`absolute top-0 left-0 w-1 h-full ${isBound ? "bg-emerald-500 shadow-lg shadow-emerald-500/20" : "bg-amber-500 animate-pulse"}`}
      ></div>
      <div className="flex justify-between items-start">
        <div className="flex items-center space-x-3">
          <div className="w-12 h-12 rounded-xl flex items-center justify-center bg-amber-500/10 border border-amber-500/20 group-hover:scale-110 transition-transform">
        ...
        <div
          className={`px-2 py-1 rounded-md text-[10px] font-black uppercase tracking-widest border ${isBound ? "text-emerald-400/80 border-emerald-400/20 bg-emerald-400/5" : "text-amber-400/80 border-amber-400/20 bg-amber-400/5"}`}
        >
          {status}
        </div>
      </div>
      <div className="text-xs text-white/60">
        Capacity: <span className="text-white font-bold">{capacity}</span>
      </div>
    </div>
  );
}
...
function DataResourceCard({
  ...
  return (
    <div className="p-6 bg-white/5 border border-white/10 rounded-2xl space-y-4 hover:bg-white/10 transition-all cursor-pointer relative group overflow-hidden">
      <div
        className={`absolute top-0 left-0 w-1 h-full ${isSecret ? "bg-red-400 shadow-lg shadow-red-400/20" : "bg-blue-400 shadow-lg shadow-blue-400/20"}`}
      ></div>
      <div className="flex justify-between items-start">
        <div className="flex items-center space-x-3">
          <div
            className={`w-12 h-12 rounded-xl flex items-center justify-center border group-hover:scale-110 transition-transform ${isSecret ? "bg-red-400/10 border-red-400/20" : "bg-blue-400/10 border-blue-400/20"}`}
          >
            {isSecret ? (
      ...
      <div className="flex flex-wrap gap-1.5 pt-2">
        {data_keys.map((k, i) => (
          <div
            key={i}
            className="px-2 py-0.5 bg-white/5 rounded border border-white/5 text-[9px] font-mono text-white/60"
          >
            {k}
          </div>
        ))}
...
function IstioCard({
  ...
  return (
    <div className="p-6 bg-white/5 border border-white/10 rounded-2xl space-y-4 hover:bg-white/10 transition-all cursor-pointer relative group overflow-hidden">
      <div
        className={`absolute top-0 left-0 w-1 h-full bg-kube-blue shadow-lg shadow-kube-blue/20`}
      ></div>
      <div className="flex justify-between items-start">
        <div className="flex items-center space-x-3">
          <div className="w-12 h-12 rounded-xl flex items-center justify-center bg-kube-blue/10 border border-kube-blue/20 group-hover:scale-110 transition-transform">
            ...
            <div className="space-y-1">
              ...
              <div className="flex flex-wrap gap-1">
                {hosts?.map((h, i) => (
                  <div
                    key={i}
                    className="px-2 py-0.5 bg-white/5 rounded border border-white/10 text-[9px] font-mono text-kube-blue"
                  >
                    {h}
                  </div>
                ))}
            ...
            <div className="space-y-1">
              ...
              <div className="flex flex-wrap gap-1">
                {gateways?.map((g, i) => (
                  <div
                    key={i}
                    className="px-2 py-0.5 bg-white/5 rounded border border-white/10 text-[9px] font-mono text-white/40"
                  >
                    {g}
                  </div>
                ))}
            ...
            <div className="space-y-1">
              ...
              <div className="space-y-1">
                {servers?.map((s, i) => (
                  <div
                    key={i}
                    className="p-2 bg-white/5 rounded border border-white/10 text-[9px] font-mono"
                  >
                    <div className="flex justify-between items-center mb-1">
                      <span className="text-white/60">{s.port?.name}</span>
                      <span className="text-kube-blue font-bold">
                        {s.port?.number}/{s.port?.protocol}
                      </span>
                    </div>
                    <div className="text-white/20 truncate">
                      {s.hosts?.join(", ")}
                    </div>
                  </div>
                ))}
...
function CertCard({
  ...
  return (
    <div className="p-6 bg-white/5 border border-white/10 rounded-2xl space-y-4 hover:bg-white/10 transition-all cursor-pointer relative group overflow-hidden">
      <div
        className={`absolute top-0 left-0 w-1 h-full ${isApproved ? "bg-emerald-500 shadow-lg" : isPending ? "bg-amber-500 animate-pulse" : "bg-red-500"}`}
      ></div>
      <div className="flex justify-between items-start">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-white/40 font-mono font-bold tracking-widest uppercase">
              {signer_name.split("/").pop()}
            </p>
          </div>
        </div>
        <div
          className={`px-2 py-1 rounded-md text-[10px] font-black uppercase tracking-widest border ${isApproved ? "text-emerald-400/80 border-emerald-400/20 bg-emerald-400/5" : "text-amber-400/80 border-amber-400/20 bg-amber-400/5"}`}
        >
          {status}
        </div>
      </div>
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-xs text-white/60">
              Learn how to create keys, CSRs, and sign certificates for Ingress.
            </p>
          </div>
        </div>
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-8 relative">
              ...
              <div className="relative pl-10">
                <div className="absolute left-0 top-0 w-8 h-8 rounded-full bg-kube-dark border-2 border-kube-blue flex items-center justify-center text-[10px] font-bold">
                  1
                </div>
                <h5 className="font-bold text-white/90">
                  Generate Private Key
                </h5>
                <p className="text-xs text-white/50 mb-2">
                  Create a 2048-bit RSA key for your workload.
                </p>
              ...
              <div className="relative pl-10">
                <div className="absolute left-0 top-0 w-8 h-8 rounded-full bg-kube-dark border-2 border-purple-500 flex items-center justify-center text-[10px] font-bold">
                  2
                </div>
                <h5 className="font-bold text-white/90">Create CSR</h5>
                <p className="text-xs text-white/50 mb-2">
                  Generate a Certificate Signing Request to be signed by the K8s
                  CA.
                </p>
                <div className="bg-black/60 rounded-lg p-3 border border-white/5 font-mono text-[10px] text-emerald-400 group">
                  ...
                  <div className="mt-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <CopyButton
                      text='openssl req -new -key server.key -out server.csr -subj "/CN=myapp.default"'
                      label="Copy Cmd"
                    />
                  </div>
                </div>
              ...
              <div className="relative pl-10">
                <div className="absolute left-0 top-0 w-8 h-8 rounded-full bg-kube-dark border-2 border-emerald-500 flex items-center justify-center text-[10px] font-bold">
                  3
                </div>
                <h5 className="font-bold text-white/90">Signing in K8s</h5>
                <p className="text-xs text-white/50 mb-2">
                  Apply the CSR to K8s and approve it.
                </p>
                <div className="bg-black/60 rounded-lg p-3 border border-white/5 font-mono text-[10px] text-emerald-400 group">
                  kubectl certificate approve my-csr-name
                  <div className="mt-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <CopyButton
                      text="kubectl certificate approve my-csr-name"
                      label="Copy Cmd"
                    />
                  </div>
                </div>
...
function HelmDesigner({ config, setConfig }: { config: any; setConfig: any }) {
  ...
  const [renderedManifests, setRenderedManifests] = useState<string>("");
  ...
  const generateValuesYaml = () => {
    return yaml.dump(config, { indent: 2, noArrayIndent: true });
  };
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        const parsed = yaml.load(event.target?.result as string) as any;
        setConfig({ ...config, ...parsed });
      } catch (err) {
        alert("Failed to parse YAML file. Please check for syntax errors.");
      }
    };
  ...
  const handleDownload = () => {
    const blob = new Blob([generateValuesYaml()], { type: "text/yaml" });
  ...
  const fetchPreview = async () => {
    ...
    setRenderError(null);
    try {
      const res = await fetch(`${apiUrl}/helm/render`, {
        method: "POST",
      ...
      if (data.error) {
        ...
        setRenderedManifests(data.manifests);
      }
    } catch (err) {
      setRenderError("Failed to connect to backend rendering service.");
    } finally {
  ...
  return (
    <div className="bg-white/5 border border-white/10 rounded-3xl overflow-hidden shadow-2xl">
      <div className="flex border-b border-white/10 bg-white/5 items-center justify-between pr-6">
        <div className="flex">
          <button
          ...
          <button
            ...
            className={`px-8 py-4 text-xs font-bold uppercase tracking-widest transition-all ${activeView === "preview" ? "bg-purple-600 text-white shadow-lg" : "text-white/40 hover:text-white hover:bg-white/5"}`}
          >
            2. Live Preview
          </button>
        </div>
        <div className="flex items-center space-x-3">
          <label className="flex items-center space-x-2 px-3 py-1.5 bg-emerald-500/10 border border-emerald-500/20 rounded-lg text-[10px] font-bold text-emerald-400 uppercase tracking-widest cursor-pointer hover:bg-emerald-500/20 transition-all">
            <Upload size={14} />
          ...
          <button
            ...
            className="flex items-center space-x-2 px-3 py-1.5 bg-kube-blue/10 border border-kube-blue/20 rounded-lg text-[10px] font-bold text-kube-blue uppercase tracking-widest hover:bg-kube-blue/20 transition-all"
          >
            <Download size={14} />
            <span>Save values.yaml</span>
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 min-h-[600px]">
        {activeView === "config" ? (
          <>
            <div className="lg:col-span-8 p-8 space-y-10 overflow-y-auto max-h-[700px] custom-scrollbar">
              {/* Metadata Section */}
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input
                      type="text"
                      value={config.fullnameOverride || "my-app"}
                      onChange={(e) =>
                        setConfig({
                          ...config,
                          fullnameOverride: e.target.value,
                        })
                      }
                      ...
                      placeholder="e.g. k8s-api-service"
                    />
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] uppercase font-bold text-white/40">
                    ...
                    <input
                      type="text"
                      value={config.namespaceOverride || "default"}
                      onChange={(e) =>
                        setConfig({
                          ...config,
                          namespaceOverride: e.target.value,
                        })
                      }
                      className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-purple-500/50"
                    />
              ...
              {/* Core Workload Section */}
              <div className="space-y-6">
                <div className="flex items-center space-x-3 text-kube-blue">
                  <Server size={20} />
                  <h4 className="text-sm font-black uppercase tracking-[0.2em]">
                    Workload & Image
                  </h4>
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-kube-blue/50"
                      placeholder="e.g. k8s-api-service"
                    />
                  </div>
                  <div className="space-y-2">
                ...
                <div className="p-6 bg-emerald-500/5 border border-emerald-500/10 rounded-2xl flex items-center justify-between">
                  <div className="space-y-0.5">
                    <span className="text-sm font-bold text-white">
                      Enable Ingress Interface
                    </span>
                    <p className="text-[10px] text-white/40">
                      Controls external access through Traefik/Nginx.
                    </p>
                  </div>
                  <button
                    onClick={() =>
                    ...
                    className={`w-12 h-6 rounded-full transition-all flex items-center px-1 ${config.ingress.enabled ? "bg-emerald-500" : "bg-white/10"}`}
                  >
                    <div
                      className={`w-4 h-4 bg-white rounded-full transition-all ${config.ingress.enabled ? "translate-x-6" : "translate-x-0"}`}
                    ></div>
                  </button>
                </div>
                {config.ingress.enabled && (
                  <div className="space-y-4 animate-in slide-in-from-top-2">
                    <div className="space-y-2">
                      ...
                      <input
                        type="text"
                        value={config.ingress.hosts[0].host}
                        onChange={(e) => {
                          const newHosts = [...config.ingress.hosts];
                          newHosts[0].host = e.target.value;
                          setConfig({
                            ...config,
                            ingress: { ...config.ingress, hosts: newHosts },
                          });
                        }}
                ...
                <div className="space-y-4">
                    ...
                    >
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          const oldVal = newEnv[key];
                          delete newEnv[key];
                          newEnv[e.target.value] = oldVal;
                          setConfig({ ...config, env: newEnv });
                        }}
                        className="flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-2 text-xs font-mono focus:border-amber-500/50"
                      />
                      <input
                        type="text"
                        value={val as string}
                  ...
                  <button
                    onClick={() =>
                      setConfig({
                        ...config,
                        env: { ...config.env, NEW_VAR: "value" },
                      })
                    }
                    className="w-full py-3 border border-dashed border-white/10 rounded-xl text-[10px] font-bold uppercase tracking-widest text-white/20 hover:text-white hover:border-white/30 transition-all flex items-center justify-center space-x-2"
                  >
                    <Plus size={14} />
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className="p-6 bg-purple-500/5 border border-purple-500/10 rounded-2xl flex items-center justify-between">
                    ...
                    <button
                      onClick={() =>
                        setConfig({
                          ...config,
                          rbac: { ...config.rbac, create: !config.rbac.create },
                        })
                      }
                      className={`w-10 h-5 rounded-full transition-all flex items-center px-1 ${config.rbac.create ? "bg-purple-500" : "bg-white/10"}`}
                    >
                      <div
                        className={`w-3 h-3 bg-white rounded-full transition-all ${config.rbac.create ? "translate-x-5" : "translate-x-0"}`}
                      ></div>
                    </button>
                  </div>
                  <div className="p-6 bg-purple-500/5 border border-purple-500/10 rounded-2xl flex items-center justify-between">
                      ...
                      onClick={() =>
                        setConfig({
                          ...
                          serviceAccount: {
                            ...
                            create: !config.serviceAccount.create,
                          },
                        })
                      }
                      className={`w-10 h-5 rounded-full transition-all flex items-center px-1 ${config.serviceAccount.create ? "bg-purple-500" : "bg-white/10"}`}
                    >
                      <div
                        className={`w-3 h-3 bg-white rounded-full transition-all ${config.serviceAccount.create ? "translate-x-5" : "translate-x-0"}`}
                      ></div>
                    </button>
                  </div>
                </div>
            ...
            <div className="lg:col-span-4 bg-black/40 border-l border-white/10 p-8 space-y-6">
              <div className="flex items-center justify-between">
                <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-white/40 italic flex items-center space-x-2">
                  <FileJson size={12} className="text-amber-400" />
                  <span>Generated Values.yaml</span>
                </h4>
                <CopyButton text={generateValuesYaml()} label="Copy YAML" />
              </div>
              <pre className="text-[10px] font-mono text-amber-100/60 leading-relaxed custom-scrollbar overflow-y-auto max-h-[600px]">
                {generateValuesYaml()}
              </pre>
            </div>
          </>
        ) : (
          <div className="lg:col-span-12 p-8 bg-[#050505] relative overflow-hidden">
            {renderLoading && (
              <div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-black/80 backdrop-blur-sm space-y-4">
                ...
                <p className="text-xs text-white/20">
                  Invoking Helm template engine via backend
                </p>
              </div>
            )}
            ...
            {renderError ? (
              <div className="h-full flex flex-col items-center justify-center text-center space-y-4">
                ...
                <div>
                  ...
                  <pre className="text-[10px] font-mono text-white/40 mt-2 bg-black/40 p-4 rounded-xl max-w-lg overflow-x-auto">
                    {renderError}
                  </pre>
                </div>
              </div>
              ...
              <div className="space-y-6">
                <div className="flex justify-between items-center text-[10px] font-bold text-white/20 uppercase tracking-[0.2em]">
                  <span>Base Chart: base-api-chart</span>
                  <span>Staging Environment: Preview Cluster</span>
                </div>
                <pre className="text-[11px] font-mono text-emerald-400/80 leading-relaxed custom-scrollbar overflow-y-auto max-h-[600px] bg-black/40 p-6 rounded-3xl border border-white/5 shadow-inner">
                  {renderedManifests || "# Click preview to render manifests"}
                </pre>
              </div>
            )}
          </div>
